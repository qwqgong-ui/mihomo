...
- A valid patched build must apply the complete patch chain, use the build tags `no_tailscale no_zerotier` (and omit `with_gvisor`), and embed a version containing `patched-p<patch-digest>`.
